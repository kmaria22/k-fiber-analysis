...
if __name__ == "__main__":
    ...
    points = data.get_segmented_points()   # numpy array: [segment_id, X, Y, Z]
    labels = data.get_labels()             # dict: fiber → segment IDs
    labels_v = data.get_vertex_labels()

    am_path_obj = Path(am_path)

    # ------------------------------------------------------------
    # KMT counts per fiber
    # ------------------------------------------------------------

    print("\nKMT counts per k-fiber:")

    fiber_kmt_counts = {}

    for fiber_name, segment_ids in labels.items():
        if fiber_name.startswith(("Pole1_", "Pole2_")):
            count = len(segment_ids)
            fiber_kmt_counts[fiber_name] = count
            print(f"{fiber_name:20} : {count:3} KMTs")

    print(f"Total k-fibers: {len(fiber_kmt_counts)}")
    ...
    for fiber_name, segment_ids in sorted(labels.items()):

        if not fiber_name.startswith(("Pole1_", "Pole2_")):
            continue

        kmt_count = len(segment_ids)

        kmt_ids_str = ", ".join(str(int(i)) for i in segment_ids)

        summary_data.append({
            "Fiber": fiber_name,
            "KMTs_per_kinetochore": kmt_count,
            "KMT_IDs": kmt_ids_str
        })

    df_summary = pd.DataFrame(summary_data)

    summary_file = am_path_obj.with_name(
        f"KMT_NO_{am_path_obj.stem}.xlsx"
    )
    ...
    # ------------------------------------------------------------
    # Create mapping KMT → Fiber
    # ------------------------------------------------------------

    print("\nCreating KMT_ID → Fiber mapping...")
    ...
    for fiber_name, segment_ids in labels.items():
        ...
        for seg_id in segment_ids:
            kmt_to_fiber[int(seg_id)] = fiber_name

    print(f"Mapped {len(kmt_to_fiber)} KMTs to fibers")

    # ------------------------------------------------------------
    # KMT length analysis
    # ------------------------------------------------------------

    print("\nStarting KMT length analysis...")

    results = []

    unique_segments = np.unique(points[:, 0])
    ...
    for seg_id in unique_segments:
        ...
        length_um = np.sum(dists) / 10000.0

        fiber = kmt_to_fiber.get(int(seg_id), "Unknown")
    ...
    if not results:
        print("No valid KMTs detected.")
        exit()

    df_lengths = pd.DataFrame(results)

    df_lengths = df_lengths.sort_values(["Fiber", "KMT_ID"])

    length_file = am_path_obj.with_name(
        f"LD_NO_{am_path_obj.stem}.xlsx"
    )

    df_lengths.to_excel(length_file, index=False)

    print("Saved:", length_file)

    # ------------------------------------------------------------
    # Fiber length calculation (NEW)
    # ------------------------------------------------------------

    print("\nComputing k-fiber lengths...")

    df_fiber_length = (
        df_lengths
        .groupby("Fiber")["Length_um"]
        .mean()
        .reset_index()
    )

    df_fiber_length = df_fiber_length.rename(columns={
        "Fiber": "K-fiber_no",
        "Length_um": "Fiber_length_um"
    })

    df_fiber_length = df_fiber_length.sort_values("K-fiber_no")

    fiber_file = am_path_obj.with_name(
        f"Fiber_Length_{am_path_obj.stem}.xlsx"
    )

    df_fiber_length.to_excel(fiber_file, index=False)

    print("Saved:", fiber_file)

    print("\nPreview:")
    print(df_fiber_length.head(10).to_string(index=False))

    # ------------------------------------------------------------
    # Pole coordinates
    # ------------------------------------------------------------

    pole1_indices = labels_v.get("Pole1", [0])
    pole1 = points[pole1_indices[0], 1:4]

    pole2_indices = labels_v.get("Pole2", [1])
    pole2 = points[pole2_indices[0], 1:4]

    print("\nPole1 coordinates:", pole1)
